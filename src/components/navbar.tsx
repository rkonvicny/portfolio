    ...
    const handleScroll = () => {
      ...
      for (const section of sections) {
        ...
        if (element) {
          ...
          // Přidáme offset pro lepší detekci
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? "bg-white/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 py-4 shadow-sm"
          : "bg-transparent py-6"
      }`}
    >
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between">
        ...
        >
          {siteTitle[0]}<span className="text-slate-900 dark:text-white">.{siteTitle[1]}</span>
        </a>
